...
export const projects = [
  {
    ...
    techDecisions: [
      {
        tech: '프레임워크 없이 직접 구현한 에이전트 루프',
        reason:
          'LangChain 같은 에이전트 프레임워크 없이 OpenAI function calling만으로 검색→판단→추가검색→최종 리뷰로 이어지는 자율 루프를 직접 구현했습니다. 반복 횟수 하드캡(6턴)과 질의당 토큰 상한(5만)도 직접 제어해, 종료 조건 없는 반복으로 API 비용이 폭주하는 상황을 막았습니다.',
      },
      {
        tech: '인용 사후검증 (citation post-verification)',
        reason:
          'LLM이 근거로 제시한 파일:라인 인용을 그대로 신뢰하지 않고, 세션 종료 시 그 파일이 실제로 존재하는지·라인 범위가 맞는지·에이전트가 이번 세션에서 진짜로 읽은 파일인지를 코드로 검증합니다. 검증에 실패한 인용은 감추지 않고 화면에 경고 배지로 그대로 노출해, "그럴듯하지만 근거 없는 지적"을 구조적으로 걸러내려 했습니다.',
      },
      {
        tech: '세션 생성과 SSE 실행을 분리, DB 폴링으로 스트리밍',
        reason:
          'POST 요청에서 곧바로 에이전트를 실행하면 프론트가 SSE를 연결하기 전에 첫 스텝들이 지나가버리는 레이스가 생깁니다. 세션 생성(PENDING)과 실행을 분리하고, SSE 연결이 원자적 CAS(UPDATE ... WHERE status=\'PENDING\')로 실행을 선점한 뒤 trace_step 테이블을 폴링해 이벤트로 변환하는 방식을 택해, 재연결 시에는 저장된 트레이스를 그대로 재생하도록 설계했습니다.',
      },
    ],
    features: [
      '자연어 질문에 search_code/search_semantic/read_file 도구를 에이전트가 자율적으로 호출해 파일:라인 인용이 달린 코드 리뷰 생성',
      'RAG — 코드를 50줄 단위(10줄 겹침)로 청킹해 OpenAI 임베딩으로 벡터화, Chroma에서 의미 기반 검색',
      'SSE 스트리밍으로 에이전트의 도구 호출("search_code 중...", "read_file 중...")을 실시간 노출, 탭을 닫아도 실행은 계속되고 재연결 시 트레이스 재생',
      '세션 종료 시 인용 사후검증 — 실패한 인용은 경고 배지로 화면에 그대로 노출',
      'LLM-as-judge 기반 eval 파이프라인으로 탐지율·오탐율·인용정확도·비용/지연 측정, Phase(정적 검색 vs RAG) 비교표 제공',
      '같은 조건을 반복 평가해 탐지율의 실행별 변동성을 점 스트립으로 시각화, 수치를 과장하지 않고 그대로 노출',
    ],
    troubleshooting: [
      {
        problem:
          'SSE 폴링이 세션 상태(status)는 COMPLETED로 반영됐는데 인용 검증 결과(citation_warnings)는 아직 NULL인 순간을 관측할 수 있는 레이스가 있었습니다. 단순히 두 기록의 순서만 바꾸는 방식으로 고쳤더니, 이번엔 그 사이에 프로세스가 죽으면 상태가 영원히 RUNNING에 멈춰버리는 새로운 크래시 윈도우가 생겼습니다.',
        solution:
          '"인용 검증 실패가 상태 기록을 막으면 안 된다"는 기존 요구사항(이를 지키는 테스트 2개가 이미 있었음)을 그대로 지키면서, 인용 기록 함수의 자체 commit을 제거하고 상태 UPDATE와 하나의 트랜잭션으로 묶어 한 번에 커밋하도록 수정했습니다. SQLite는 같은 트랜잭션 안에서 한 statement가 실패해도 이후 statement 실행은 막지 않는다는 점을 직접 확인하고 적용해, 레이스와 크래시 윈도우를 동시에 없앴습니다.',
      },
      {
        problem:
          '코드 리뷰에서 위 문제를 "하나의 UPDATE로 합치라"는 방식으로 고치라는 제안을 받았는데, 그대로 적용하면 인용 관련 컬럼 기록이 실패할 경우 상태 UPDATE까지 함께 실패해 오히려 예전에 이미 막아뒀던 회귀(인용 검증 실패가 세션 전체를 실패로 만드는 문제)가 재발할 상황이었습니다.',
        solution:
          '제안을 그대로 따르지 않고, 기존에 이 회귀를 막기 위해 작성된 테스트 2개의 monkeypatch 전제를 먼저 추적했습니다. 그 결과 "재정렬 + 트랜잭션 통합"이라는 다른 방식을 선택해, 리뷰가 지적한 문제와 기존에 지켜야 했던 요구사항을 둘 다 만족시켰습니다.',
      },
    ],
    retrospective:
      'AI 에이전트가 "그럴듯한 답"과 "근거 있는 답"을 구분하지 못하면 실무에서는 오히려 위험할 수 있다는 문제의식에서 시작한 프로젝트입니다. 인용 사후검증처럼 LLM 출력을 코드로 다시 검증하는 안전장치를 직접 설계하고, eval에서도 탐지율 하나만 자랑하지 않고 오탐율·인용정확도·재실행 변동성까지 같이 측정하면서, "AI 기능을 붙이는 것"과 "그 기능을 신뢰할 수 있게 만드는 것"은 전혀 다른 작업이라는 걸 체감했습니다. 코드 리뷰 제안을 그대로 받아들이지 않고 기존 테스트의 전제를 먼저 추적해 더 나은 해법을 찾은 경험도, 리뷰 피드백을 검증 없이 적용하면 안 된다는 걸 다시 확인한 계기였습니다. 다음에는 eval 셋 규모를 키워 Phase 간 탐지율 차이가 통계적으로도 유의미한지 확인해보고 싶습니다.',
    links: [{ label: 'GitHub', href: 'https://github.com/chobo6/repoview' }],
  },
  {
    title: 'Songpyeon',
    summary:
